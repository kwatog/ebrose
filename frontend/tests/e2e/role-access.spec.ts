...
test.describe('Role-Based Access Control - True E2E', () => {
  test('Viewer role can see budget items', async ({ page }) => {
    ...
    // BaseTable or empty state should be visible
    await expect(page.locator('.base-card, .empty-state, h3').first()).toBeVisible({ timeout: 10000 });
  });
  ...
  test('Manager can access budget items', async ({ page }) => {
    ...
    await expect(page.locator('.base-table, .empty-state')).toBeVisible({ timeout: 10000 });
  });
...
test.describe('Role Permission Matrix - True E2E', () => {
  ...
  permissions.forEach(({ role, create, read, update, delete: del, admin }) => {
    test(`${role} permissions: C:${create} R:${read} U:${update} D:${del} Admin:${admin}`, async ({ browser }) => {
      ...
      if (admin) {
        await expect(page.locator('button.nav-link:has-text("Admin")')).toBeVisible();
      } else {
        await expect(page.locator('button.nav-link:has-text("Admin")')).not.toBeVisible();
      }
